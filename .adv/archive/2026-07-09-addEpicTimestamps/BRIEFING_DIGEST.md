...
**Status:** archived
**Generated:** 2026-07-09T00:44:47.143Z
...
- **[agenda]** follow_ups: Consider adding an explicit spec scenario under rq-epicCliList01 asserting each Epic entry includes an ISO UTC startTime, to make the acceptance criterion machine-checkable rather than relying on 'at least id'.
- **[agenda]** follow_ups: Confirm desired field semantics in acceptance: startTime = Temporal workflow start (Epic workflow creation), documented as such, not a mutable last-activity timestamp.
- **[archive_only_evidence]** sources: Temporal TS SDK type — WorkflowExecutionInfo: WorkflowExecutionInfo has non-optional `startTime: Date` (line 33) and optional `executionTime?: Date` (line 34), `closeTime?: Date` (line 35). These are part of each list row already; no describe/query needed.
- **[archive_only_evidence]** sources: Temporal TS SDK — WorkflowClient.list() yields full info rows: `AsyncWorkflowListIterable extends AsyncIterable<WorkflowExecutionInfo>` and `_list(): AsyncIterable<WorkflowExecutionInfo>`. Iterating list() gives each row's startTime with zero extra RPCs — Visibility returns it in the ListWorkflowExecutions response.
- **[archive_only_evidence]** sources: Current CLI payload builder — only emits { id }: EpicListEntry = { id } and buildLiveEpicListPayload maps ids to `{ id }`. listEpicIdsFromVisibility returns string[] — startTime is discarded upstream.
- **[archive_only_evidence]** sources: Visibility enumerator drops all fields except workflowId: ListEpicClient.workflow.list iterator is typed `AsyncIterable<{ workflowId: string }>` and listEpicWorkflowIds returns string[]. The lossy narrowing to string[] here is the structural reason startTime never reaches the CLI — must return {id, startTime} tuples instead.
- **[archive_only_evidence]** sources: Spec rq-epicCliList01 — read-only/worker-free/no-hydration boundary: MUST use Visibility enumeration; MUST NOT read ADV state files; MUST NOT query or hydrate each Epic workflow; MUST NOT require a worker; scenario .1 requires each entry `object containing at least id` (additive-safe); scenario .4: `list path does not query per-Epic workflow state`.
...
- **[agenda]** follow_ups: Add an explicit spec scenario under rq-epicCliList01 asserting each Epic entry includes an ISO-8601 UTC startTime (or null), to make AC1/AC2 machine-checkable rather than relying on scenario .1 'at least id'.
- **[agenda]** follow_ups: Document startTime semantics in the payload/spec as Temporal workflow start time (Epic list timestamp source), not child-change last activity, to prevent consumer misread (agreement DONT1).
- **[archive_only_evidence]** sources: Temporal TS SDK type — WorkflowExecutionInfo.startTime non-optional: startTime: Date (non-optional, line 33); executionTime?: Date and closeTime?: Date optional (lines 34-35). Confirms design Key Decision #1 and Avoidance DONT2 (do not depend on optional executionTime).
- **[archive_only_evidence]** sources: Temporal TS SDK runtime deserializer treats startTime as required: startTime is parsed via requiredTsToDate('startTime') (throws if missing), while executionTime/closeTime use optionalTsToDate. startTime is structurally guaranteed on every parsed Visibility list row — no describe/query RPC needed.
...
- **[archive_only_evidence]** sources: Spec rq-epicRetiredListing01 — active-only structural filter: adv epic list --json MUST enumerate live epicWorkflow with AdvEpicStatus = 'active', ExecutionStatus = 'Running', and project-prefix filtering. Design AC3 preserves this exact query — no filter change.
- **[archive_only_evidence]** sources: Source — current lossy narrowing at Visibility enumerator: ListEpicClient.workflow.list iterator typed AsyncIterable<{ workflowId: string }> and listEpicWorkflowIds returns string[]. Row startTime is discarded here — the structural reason it never reaches the CLI. Design widens iterator + adds rich helper listEpicWorkflows() returning {id,startTime}, keeping listEpicWorkflowIds() as string[] wrapper.
- **[archive_only_evidence]** sources: Source — current CLI payload builder emits only { id }: EpicListEntry = { id }; buildLiveEpicListPayload maps ids.map(id => ({id})). Design changes EpicListEntry to { id; startTime: string | null } and maps Date -> toISOString(), null preserved. Fail-closed path (buildLiveEpicListFailure, epics:[]) unaffected.
