...
Every task MUST have a normalized evidence plan that records exactly one evidence policy, one non-empty `proof_target`, and explicit compatibility provenance. New and materially reclassified tasks carry provenance `new` or `reclassified`; legacy in-flight tasks without plan provenance are normalized on read with compatibility `legacy`. Title, path, and test-quality heuristics (assertion density, mock surface, coverage, flake indicators) are advisory inputs only and MUST NOT determine the plan or gate task completion. Behavior-critical tasks (type `code` or `verification`) MUST NOT use `not_applicable` as their evidence route; a non-test route for logic-bearing work requires a bounded rationale (at most 500 non-whitespace characters) and a linked `review_conclusion`.

**Tags:** `tdd`, `evidence`, `task-model`, `structural-correctness`

#### Scenarios

**New task carries exactly one policy and proof target** (`rq-TDD013evp.1`)
...
- The task's evidence plan uses policy `not_applicable`

**When:** The plan is validated at prep or completion

**Then:**

- The plan is rejected as invalid
- The task is directed to a proof-bearing route such as test, review, or static_check

**Non-test route for logic-bearing work requires rationale and review conclusion** (`rq-TDD013evp.3`)

**Given:**

- A behavior-critical task selects a non-test evidence policy
- The task is new or materially reclassified

**When:** The plan is validated

**Then:**

- A bounded rationale (<= 500 non-whitespace characters) is present
- A linked review_conclusion is present
- The task is blocked until both are provided

**Legacy task without plan provenance normalizes on read** (`rq-TDD013evp.4`)
...
- The plan is materialized with compatibility `legacy`
- No heuristic cutover is performed
- Valid historical evidence remains readable
