...
## Phase 4: Recon Burst
Use `explore` via Task tool for the bounded read-only search burst, or work inline if `explore` is unavailable. Pass only:
...
| Recon burst | Task tool (`explore`) or inline fallback |
