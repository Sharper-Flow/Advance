...
- **Actual pack consumption — uninstrumented adoption.** No durable counter records which discoveries cited which pack, or whether a pack changed an agreement. Pack usefulness therefore cannot be confirmed or denied from durable state; only the production obligation, the discovery citation obligation, and 4 on-disk packs are observable.
