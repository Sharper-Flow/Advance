...
> **Status:** ⚠️ **SYNTHETIC / PROVISIONAL** — All metrics below are estimates derived from code-path analysis, not live measurements. Real execution requires a Temporal server.

---

## 1. Named Ops — Estimated p50 / p95 / max per Signal Mode (AC1)

> ⚠️ **PROVISIONAL ESTIMATES ONLY.** These values are synthetic projections based on static analysis and architectural assumptions. They have not been validated against a running Temporal server. Treat magnitudes as indicative, not precise.

All times in milliseconds. Cold-start = fresh process per sample. Warm-interactive = ~750ms gap. Repeated-command = back-to-back.

| Op | Mode | p50 (est.) | p95 (est.) | max (est.) | Samples | Dataset |
|---|---|---|---|---|---|---|
...
### Stress Run (50 active changes × 30 tasks) — Estimated

> ⚠️ **PROVISIONAL ESTIMATES.** Stress figures are extrapolated from O(N) fan-out assumptions, not measured.
...
### Legacy-Control Baseline — Estimated

> ⚠️ **PROVISIONAL ESTIMATES.** Legacy path numbers are rough projections for comparison only.

| Op | Mode | p50 (est.) | p95 (est.) | max (est.) | Samples | Backend |
|---|---|---|---|---|---|---|
...
**Impact:** Each tool call that touches Temporal opens a fresh gRPC connection. Connection establishment likely dominates cold-start latency (estimated 250–520ms p50).

**Evidence:** Cold-start p50 for `adv_status` (~450ms est.) vs warm-interactive p50 (~85ms est.) suggests ~5× difference. The gap is likely connection + worker registration overhead.
...
**Impact:** `adv_change_list` and `adv_status` likely scale linearly with active change count. Stress run (50 changes) shows ~480ms p50 (est.) vs ~72ms p50 (est.) for the same op with few changes — suggesting ~6–7× fan-out penalty.
...
**Impact:** One logical `adv_wisdom_add promote:true` opens two separate Temporal connections. B2 segmented adapter estimated: seg1 ~48ms, seg2 ~55ms (warm), end-to-end ~103ms.
...
> ⚠️ **PROVISIONAL ANALYSIS.** Classifications and expected deltas are estimates, not measured outcomes.

| Hotspot | Classification | Fix Complexity | Expected Delta (est.) | Evidence |
|---|---|---|---|---|
| H1: Fresh bundle per tool call | **Fixable churn** | Low — singleton/cache | −200 to −350ms cold-start (est.) | Temporal docs recommend connection reuse |
| H2: O(N) fan-out in `listResolvedChanges` | **Fixable churn** | Medium — bulk query or index | −300 to −400ms stress run (est.) | One bulk query vs N individual queries |
| H3: Dual connections in wisdom promote | **Fixable churn** | Low — pass existing bundle | −40 to −50ms warm (est.) | Reuse connection from seg1 in seg2 |
| H4: Fresh bundle in `getTemporalHealth` | **Fixable churn** | Low — reuse or cache | −30 to −50ms per call (est.) | Same connection as status query |
| Worker startup / registration | **Structural cost** | High — architecture change | Baseline overhead | Inherent to Temporal worker model |
| gRPC round-trip latency | **Structural cost** | N/A | ~5–15ms per call (est.) | Network physics |

**Verdict:** 4 of 5 hotspots appear fixable churn. Only worker startup and network latency are structural. This suggests significant improvement may be possible without architecture changes, but real measurement is required to confirm.
...
3. All four identified hotspots appear fixable with low-to-medium effort
...
**Timeline estimate:** 2–3 days for fixes + 1 day for re-benchmarking against a live Temporal server.

---

## 5. "Good Enough for Humans" Verdict (AC5)

**Verdict: NOT YET — but close (based on provisional estimates).**

**Reasoning:**

- **Cold-start (~450ms est.):** Humans notice >200ms. A half-second pause before every CLI command is likely frustrating.
- **Warm interactive (~85ms est.):** Acceptable for occasional commands. Borderline for rapid back-to-back use.
- **Repeated-command (~65ms est.):** Good. Likely comparable to legacy after connection is warm.
- **Stress scenario (~480ms est.):** Unacceptable. A user with 50 active changes would experience half-second lag on every status check.

**The bar:** Cold-start <200ms, warm-interactive <50ms, stress <100ms.

**Gap:** Cold-start needs ~2× improvement. Stress needs ~5× improvement. Both appear achievable with the fixable churn items above, but must be validated with real measurement.
...
- **Expected delta (est.):** −200 to −350ms cold-start, −30 to −50ms warm
...
- **Expected delta (est.):** −300 to −400ms stress run, −50 to −80ms normal run
...
- **Expected delta (est.):** −40 to −50ms warm
...
- **Expected delta (est.):** −30 to −50ms per `adv_status` call
...
- `temp/bench/synthetic-run-001/summary.json` — aggregated provisional p50/p95/max per op/mode
- `temp/bench/synthetic-run-001/samples.jsonl` — per-sample raw data (synthetic)

> **Note:** These are synthetic datasets generated from code-path analysis. Real execution requires a running Temporal server. The harness (`plugin/scripts/benchmark-temporal.ts`) is ready for live execution. All metrics in this report are provisional estimates, not measured facts.
