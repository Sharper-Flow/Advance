...
| `adv_store_reconcile` | operator-only | Matching dry-run `plan_hash`; target trust approval for cross-project apply | Repairs disk-backed migration residue; plan/dry_run is read-only and apply re-verifies the fresh plan before mutation (`AC12`, `C5`) |
