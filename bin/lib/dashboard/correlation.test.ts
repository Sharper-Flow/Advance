...
const changes = [
  ...
  },
  {
    id: "otherChange",
    title: "Other change",
    correlation_keys: { branches: ["change/otherChange"], head_shas: ["def456"] },
  },
];
...
describe("dashboard correlation", () => {
  test("links PRs, workflow runs, deployments, and ops evidence structurally", () => {
    const result = correlateDashboardItems({
      changes,
      pulls: [{ number: 7, head: { ref: "change/addLocalDashboard", sha: "abc123" } }],
      workflow_runs: [{ id: 8, head_branch: "change/addLocalDashboard", head_sha: "abc123" }],
      deployments: [{ id: 9, ref: "change/addLocalDashboard", sha: "abc123", environment: "prod" }],
      ops: [{ id: "ops-1", environment: "prod", completion_signal: "dashboard-ready" }],
    });
  ...
  test("keeps unknown and ambiguous activity in unlinked lane", () => {
    const result = correlateDashboardItems({
      changes: [
        ...changes,
        { id: "duplicate", title: "Duplicate", correlation_keys: { branches: ["change/addLocalDashboard"], head_shas: [] } },
      ],
      pulls: [
        { number: 1, head: { ref: "change/missing", sha: "zzz999" } },
        { number: 2, head: { ref: "change/addLocalDashboard", sha: "nomatch" } },
      ],
      workflow_runs: [],
      deployments: [],
      ops: [],
