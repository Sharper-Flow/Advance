...
- **Size bound & repair-on-reject (AC2):** Every free-text field in your report must stay within the per-lane size bound of **4000 chars**. If `adv_subagent_report_submit` rejects your report with a size-bound error naming an offending field, condense that field within your own context and retry the submit **exactly once**. If it is still rejected after that one repair pass, return an explicit failure naming the rejected field — do **not** silently truncate or head/tail-excerpt the field to force acceptance.
