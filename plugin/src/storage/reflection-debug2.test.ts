...
import { appendReflection, getReflectionsPath } from "./reflection";

describe("debug2", () => {
  test("check file content", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "reflection-debug2-"));
    ...
    console.log("Parsed:", JSON.parse(content.trim()));
    console.log("Type of proposal:", typeof JSON.parse(content.trim()).plane1.efficiency.per_gate_ms.proposal);
