...
describe("navigateConflicts", () => {
  ...
  const makeDeps = (
    overrides: Partial<NavigateConflictsDeps> = {},
  ): NavigateConflictsDeps => ({
    prompt: vi.fn().mockResolvedValue("auto"),
    apply: vi
      .fn()
      .mockResolvedValue({ ok: true, action: "skip", auditEntry: "audit" }),
  ...
  it("presents batch summary with correct counts", async () => {
    ...
    const promptCall = (deps.prompt as ReturnType<typeof vi.fn>).mock
      .calls[0][0] as string;
  ...
  it("auto mode applies auto-skip, auto-resolve, then resolveDivergent for each divergent", async () => {
    ...
    const deps = makeDeps({
      ...
      apply: vi
        .fn()
        .mockImplementation(
          (_action, _filePath): Promise<ResolveActionResult> => {
            if (_action.kind === "skip") {
              return Promise.resolve({
                ok: true,
                action: "skip",
                auditEntry: "skipped: dup",
              });
            }
    ...
    expect(deps.apply).toHaveBeenNthCalledWith(
      1,
      { kind: "skip", reason: "duplicate_content reason" },
      "dup.ts",
      repoRoot,
    );
  ...
  it("step mode calls resolveDivergent + apply for every conflict in order", async () => {
    const conflicts: ConflictRecord[] = [
      makeConflict("a.ts", "duplicate_content"),
      makeConflict("b.ts", "auto_resolvable_trivial", [
        { ours: "x", theirs: "x " },
      ]),
      makeConflict("c.ts", "divergent_content", [
        { ours: "old", theirs: "new" },
      ]),
    ];
  ...
  it("abort mode applies abort_rebase exactly once and returns mode abort", async () => {
    const conflicts: ConflictRecord[] = [
      makeConflict("a.ts", "divergent_content", [
        { ours: "old", theirs: "new" },
      ]),
    ];
  ...
  it("stops applying when an apply fails and does not process remaining conflicts", async () => {
    ...
    expect(deps.apply).not.toHaveBeenCalledWith(
      expect.any(Object),
      "c.ts",
      repoRoot,
    );
  ...
  it("stops and returns aborted when resolveDivergent returns abort_rebase in auto mode", async () => {
    const conflicts: ConflictRecord[] = [
      makeConflict("a.ts", "duplicate_content"),
      makeConflict("b.ts", "divergent_content", [
        { ours: "old", theirs: "new" },
      ]),
      makeConflict("c.ts", "divergent_content", [
        { ours: "old2", theirs: "new2" },
      ]),
    ];
  ...
  it("stops and returns unresolved when resolveDivergent returns abort_rebase in step mode", async () => {
    ...
    const deps = makeDeps({
      ...
      resolveDivergent: vi
        .fn()
        .mockImplementation((conflict): Promise<ResolveAction> => {
  ...
  it("uses current iteration index when duplicate conflict references abort in step mode", async () => {
    const duplicate = makeConflict("dup.ts", "divergent_content", [
      { ours: "old", theirs: "new" },
    ]);
    const remaining = makeConflict("remaining.ts", "divergent_content", [
      { ours: "old2", theirs: "new2" },
    ]);
    const conflicts: ConflictRecord[] = [duplicate, duplicate, remaining];
    ...
    expect(result.ok).toBe(false);
    expect(result.unresolved).toEqual([remaining]);
  });
});
