...
- Impact wording is evidence-only. Do not invent user/business benefit — but DO trace and surface benefit prose already articulated in the proposal, agreement, or executive summary (canonical source: `_executiveSummary ## Value`). Evidenced benefit MUST appear in `release_notes.headline_internal`; leaving evidenced benefit untraced in the release notes is a gap to flag, not a safe default. If no value articulation exists in any artifact, `headline_internal` may remain absent.
...
- After the executive summary is persisted, full-replace or refine the existing release notes block via `adv_change_set_release_notes changeId: {id} release_notes: { ... }`. Read current `change.release_notes` from `adv_change_show`; preserve its optional narrative and keep it as one `ReleaseNotesContent` block. Apply harden evidence to add or refine fields such as `breaking`, `area`, or `links.pr` only when now known. Do not ask a new question; unsupported semantic fields remain absent. The release pipeline (`auto-release.yml`) renders `headline_internal` and `highlights[]` into the user-visible release body; empty fields produce empty highlights. Per the rule above, the agent MUST populate `headline_internal` and at least one `highlights[]` entry whenever the artifacts evidence user-facing benefit.
