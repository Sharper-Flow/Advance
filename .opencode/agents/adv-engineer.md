...
Behavior-bearing inline code retains TDD and must produce red/green evidence. For `separate_verification`, follow the declared test, review, or static-check route. Record the selected proof in the report: test execution uses its `adv_run_test` run ID; a static check records its command and result without a test run ID.
