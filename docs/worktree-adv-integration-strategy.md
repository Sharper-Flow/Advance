...
**Date:** 2026-05-02
**Status:** Design proposal — pending user direction
...
```ts
async function detectStaleHead(repoRoot: string): Promise<StaleHeadReport | null> {
  ...
  const mergedToDefault = (await execGit("branch", "--merged", `origin/${defaultBranch}`))
    .split("\n").map(l => l.trim().replace(/^\* /, "")).includes(head);
  ...
  if (advChangeId) {
    ...
    if (!change || change.status === "archived" || change.status === "closed") {
      return { branch: head, reason: "adv-change-archived-or-missing", suggestion: `Switch to ${defaultBranch}.` };
    }
  }
...
*Generated 2026-05-02. Authored after the multi-hour pokeedge cleanup session and the worktree-instruction audit (`docs/worktree-instruction-audit.md`).*
