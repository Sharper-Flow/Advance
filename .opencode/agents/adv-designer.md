...
Behavior-bearing inline UI code retains TDD and must produce red/green evidence. Record the selected proof in the report; use an `adv_run_test` run ID only when the selected route executes a test or static check.
