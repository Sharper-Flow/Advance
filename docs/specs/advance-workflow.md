...
**When:** The live PR title has no conventional `<type>:` prefix, its parsed type is outside `allowed_types`, its parsed type is outside configured `release_types`, or the live-title lookup fails

**Then:**
- armPullRequestAutoMerge returns `PR_TITLE_POLICY_VIOLATION` for a missing prefix or parsed type policy violation, returns `PR_TITLE_LOOKUP_FAILED` for a live-title lookup failure, and does NOT arm auto-merge
- armPullRequestAutoMerge does not return `PR_TITLE_TYPE_UNRESOLVED`, which remains a PR creation-path blocker
