...
export function correlateDashboardItems(input: CorrelateDashboardInput): CorrelateDashboardResult {
  const index = buildCorrelationIndex(input.changes);
  const linked: LinkedDashboardItem[] = [];
  const unlinked: UnlinkedDashboardItem[] = [];

  for (const pull of input.pulls) correlateOne("pull", pull, pullEvidence(pull), index, linked, unlinked);
  for (const run of input.workflow_runs)
    correlateOne("workflow_run", run, runEvidence(run), index, linked, unlinked);
  for (const deployment of input.deployments)
    correlateOne("deployment", deployment, deploymentEvidence(deployment), index, linked, unlinked);
  for (const ops of input.ops) correlateOne("ops", ops, opsEvidence(ops), index, linked, unlinked);
...
function buildCorrelationIndex(changes: CorrelatableChange[]): CorrelationIndex {
  const index: CorrelationIndex = { branches: new Map(), shas: new Map(), ops: new Map() };
  for (const change of changes) {
    for (const branch of change.correlation_keys?.branches ?? []) add(index.branches, branch, change.id);
    for (const sha of change.correlation_keys?.head_shas ?? []) add(index.shas, sha, change.id);
...
function correlateOne(
  ...
  unlinked.push({ kind, item, reason: sawAmbiguous ? "ambiguous structural match" : "no structural match", status: statusOf(item) });
}

function lookup(index: CorrelationIndex, evidence: CandidateEvidence): string[] {
  if (evidence.key === "branch") return index.branches.get(evidence.value) ?? [];
...
function runEvidence(run: unknown): CandidateEvidence[] {
  return compact([
    evidence("branch", stringField(record(run), "head_branch"), "run.head_branch"),
...
function opsEvidence(ops: unknown): CandidateEvidence[] {
  ...
  return key ? [{ key: "ops", value: key, label: "ops.environment+completion_signal" }] : [];
}
...
function evidence(key: string, value: string | undefined, label: string): CandidateEvidence | undefined {
...
function statusOf(item: unknown): string | undefined {
  return stringField(record(item), "status") ?? stringField(record(item), "state");
}
