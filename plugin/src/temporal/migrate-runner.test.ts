...
import {
  loadMigrationProjectInput,
  migrateSingleProjectActivity,
  runMigrationSweep,
} from "./migrate-runner";
...
describe("temporal migrate runner", () => {
  ...
  it("runMigrationSweep starts migrateAllProjectsWorkflow on the control-project task queue and falls back to getHandle when already started", async () => {
    ...
    const result = await runMigrationSweep(
      ...
      {
        controlProjectId: "control-proj",
        runId: "cutover-1",
        projectPaths: ["/tmp/p1", "/tmp/p2"],
      },
    );
    ...
    expect(start).toHaveBeenCalledWith(
      expect.any(Function),
      expect.objectContaining({
        workflowId: "adv/migration/control-proj/cutover-1",
        taskQueue: "advance-control-proj",
      }),
    );
    expect(getHandle).toHaveBeenCalledWith(
      "adv/migration/control-proj/cutover-1",
    );
