...
ADV today has no way to **detect** that the snapshot store is degraded. The forensic pattern (find stale locks via `find`, scan zero-byte objects, run `git fsck` on each bare repo) has been executed manually in at least four diagnosis sessions for example-web alone (2026-05-04, 2026-05-10, 2026-05-11, 2026-05-12). Every recurrence requires re-investigation from scratch.
