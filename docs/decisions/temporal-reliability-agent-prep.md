...
| T3 | ~~MEDIUM~~ **CLOSED 2026-05** | ~~No load test for concurrent agent sessions.~~ Closed via `unifyworktreeunderadvmultisess` (T38–T39). See `plugin/src/__tests__/temporal-concurrent-load.itest.ts` for stress scenarios A+B+C. Validated 5+ concurrent clients with worker-lock contention, state-write race (5×10 worktree register/remove cycles), and worker-kill respawn-elect (rq-workerSingleton01.3 stale-PID reclaim). | `plugin/scripts/benchmark-temporal.ts --mode=concurrent-clients` (T38 harness extension); opt-in via `RUN_INTEGRATION_TESTS=1 pnpm test src/__tests__/temporal-concurrent-load.itest.ts` |
