...
const manifest = {
  ...
  conversion_errors: 1,
  tools: {
    adv_allowed: { status: "available" as const, schema_bytes: 32, approx_tokens_4char_rule: 8 },
    adv_denied: { status: "available" as const, schema_bytes: 13, approx_tokens_4char_rule: 4 },
    adv_failed: { status: "conversion_error" as const, schema_bytes: null, approx_tokens_4char_rule: null, conversion_error: "bad" },
  },
};

describe("tool schema projection", () => {
  it("projects only tools allowed by OpenCode's resolved permission profile", () => {
    expect(projectToolSchemaManifest(manifest, { adv_allowed: true, adv_denied: false })).toEqual({
