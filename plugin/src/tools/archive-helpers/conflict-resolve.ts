...
export type ResolveActionResult =
  ...
  | {
      ok: false;
      error: "GIT_FAILED" | "WRITE_FAILED" | "REBASE_ABORT_FAILED";
      detail: string;
    };
  ...
  switch (action.kind) {
    case "skip": {
      ...
      if (!result.ok) {
        return {
          ok: false,
          error: "GIT_FAILED",
          detail: result.error ?? "git rebase --skip failed",
        };
      }
      return {
        ok: true,
        action: "skip",
        auditEntry: `skipped: ${action.reason}`,
      };
    }
    ...
    case "auto_resolve": {
      ...
      if (!addResult.ok) {
        return {
          ok: false,
          error: "GIT_FAILED",
          detail: addResult.error ?? "git add failed",
        };
      }
      ...
      if (!contResult.ok) {
        return {
          ok: false,
          error: "GIT_FAILED",
          detail: contResult.error ?? "git rebase --continue failed",
        };
      }
      return {
        ok: true,
        action: "auto_resolve",
        auditEntry: `auto-resolved: ${action.reason}`,
      };
    }
    ...
    case "user_resolve_in_place": {
      ...
      return {
        ...
        auditEntry: `user-resolved-in-place: ${action.userReason}`,
      };
    }
    ...
    case "abort_rebase": {
      ...
      if (!result.ok) {
        return {
          ok: false,
          error: "REBASE_ABORT_FAILED",
          detail: result.error ?? "git rebase --abort failed",
        };
      }
      return {
        ok: true,
        action: "abort_rebase",
        auditEntry: `rebase-aborted: ${action.userReason}`,
      };
    }
    ...
    case "skip_with_decision": {
      ...
      return {
        ok: true,
        action: "skip_with_decision",
        auditEntry: `skipped-with-decision: ${action.userReason}`,
      };
    }
    ...
    default: {
      ...
      return {
        ok: false,
        error: "GIT_FAILED",
        detail: `Unknown action kind: ${(action as ResolveAction).kind}`,
      };
    }
  }
}
...
async function defaultGitRebaseSkip(
  repoRoot: string,
): Promise<{ ok: boolean; error?: string }> {
  try {
...
async function defaultGitRebaseContinue(
  repoRoot: string,
): Promise<{ ok: boolean; error?: string }> {
  try {
...
async function defaultGitRebaseAbort(
  repoRoot: string,
): Promise<{ ok: boolean; error?: string }> {
  try {
