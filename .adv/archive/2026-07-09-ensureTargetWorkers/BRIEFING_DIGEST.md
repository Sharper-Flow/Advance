...
**Status:** archived
**Generated:** 2026-07-09T18:12:06.720Z
...
Showing 56 of 56 durable facts.
...
- **[agenda]** follow_ups: Next task (tk-1fa6281f46f2) should remove the NotImplemented guard and implement target project trust resolution + bounded serviceability verification for adv_temporal_worker_restart target_path.
...
- **[agenda]** follow_ups: Disk-snapshot read authority metadata (AC5/AC6/DONT2/DONT3) is intentionally out of scope for this task and remains for the following task before acceptance gate can complete.
...
- **[unresolved_action]** consumer_warnings: verification_missing: No adv_run_test evidence found for reported command: pnpm run check
- **[agenda]** follow_ups: Confirm with user whether ensureTargetWorkers scope includes lightweight IPC queue-registration ('ensure') or only full target worker restart.
- **[agenda]** follow_ups: FIELD_POLICIES: add adv_temporal_worker_restart target_path/target_confirmed/confirmationEvidence blank:omit entries (preflight test at tool-arg-preflight.test.ts:1577 must extend).
...
- **[archive_only_evidence]** sources: plugin/src/temporal/out-of-process-worker.ts:61-62 + worker-multi.ts:461 registerQueue: registerQueue sends an IPC register message to the existing worker child rather than spawning a new child; tests (out-of-process-worker.test.ts:193) confirm 'sends IPC message instead of spawning new child'.
- **[archive_only_evidence]** sources: plugin/src/tools/temporal-ops.ts:498-553 adv_temporal_reconnect target_path pattern: Reference implementation of target_path/target_confirmed/confirmationEvidence on a temporal-ops tool: wraps runReconnect in withTargetPathStore(stateRequirement:'temporal-required') and appends formatTargetProjectContext(context) as _projectContext.
- **[archive_only_evidence]** sources: plugin/src/tools/target-project.ts:218-268 ensureTargetMutationQueueReady: temporal-required store path throws TargetProjectError when the target queue is NOT serviceable. This directly conflicts with restart semantics, whose precondition is an unserviceable/wedged queue.
- **[archive_only_evidence]** sources: plugin/src/tools/target-project.ts:337-353 formatTargetProjectContext: Currently emits a warning only for untrusted targets. Draft AC requires disk_snapshot_non_authoritative authority metadata for ALL snapshot-ok reads including trusted ones.
- **[archive_only_evidence]** sources: plugin/src/tools/temporal-ops.ts:589-651 restart handler derives projectId from store.paths.root: restartCurrentProjectTemporalWorker(projectDir) derives projectId+queue from the passed dir; target-awareness = pass context.root instead of store.paths.root. Minimal seam.
- **[archive_only_evidence]** sources: advance-meta spec rq-statusCliWorkerFree01 / rq-dashboardWorkerFree01: Canonical ADV pattern: live-Temporal reads fail closed; disk projections MUST NOT substitute for live active state. Establishes the precedent for disk_snapshot_non_authoritative authority metadata.
...
- **[agenda]** follow_ups: Add planning task + AC/test: after adv_temporal_worker_restart target full-restart, the driving (source) project queue remains serviceable (re-register via ensureProjectTemporalQueue(sourceProjectId) or prove auto-recovery). Ties to plugin-init.ts:619,633.
- **[agenda]** follow_ups: Confirm FIELD_POLICIES.adv_temporal_worker_restart adds target_path/target_confirmed/confirmationEvidence blank:omit and extends preflight test at tool-arg-preflight.ts:1577 (design step 2 already scopes this).
- **[agenda]** follow_ups: Traceability note: AC1/AC2 presume adv_temporal_worker_restart#target_path (built by this change) with no declared [warrant:] tag — compliant under rq-acWarrant01 proportionality; no action needed.
...
- **[archive_only_evidence]** sources: plugin/src/plugin-init.ts:462-476 ensureProjectTemporalQueue: Registers a target queue onto the already-running in-process worker via worker.registerQueue(queue); throws if no worker registered. Confirms KD4 cheap ensure path exists and is additive (does not drop existing queues). registerQueue is idempotent (out-of-process-worker.test.ts:218).
- **[archive_only_evidence]** sources: plugin/src/tools/target-project.ts:218-335 ensureTargetMutationQueueReady / withTargetPathStore: temporal-required store path throws TargetProjectError when the target queue is NOT serviceable (line 265). Restart's precondition IS unserviceable, so KD2 (resolveTargetProject direct, not withTargetPathStore temporal-required) is correct — reusing the temporal-required gate would reject exactly the case restart must repair.
- **[archive_only_evidence]** sources: plugin/src/tools/temporal-ops.ts:556-703 adv_temporal_worker_restart current implementation: Current args are approvedLockReclaim/approvalEvidence only; projectId derived from store.paths.external||store.paths.root; success requires waitForRestartServiceability + isRestartServiceabilityVerified fresh proof. Failure path already returns structured serviceability/_freshness/worker_lock/stsl/expectedQueue. Confirms KD5 shape is already the established pattern; target extension is additive.
- **[archive_only_evidence]** sources: plugin/src/tools/temporal-ops.ts:498-553 adv_temporal_reconnect target_path precedent: Shipped reference for target_path/target_confirmed/confirmationEvidence on a temporal-ops tool, appending formatTargetProjectContext(context) as _projectContext. Confirms KD1/KD3 trust-contract reuse is the boring, proven path.
- **[archive_only_evidence]** sources: plugin/src/utils/tool-arg-preflight.ts:268-273 FIELD_POLICIES precedent: adv_temporal_reconnect already declares target_path+confirmationEvidence blank:omit; adv_temporal_worker_restart entry (line 273) exists but lacks target fields. Confirms design implementation step 2 (FIELD_POLICIES omit handling) is real and its preflight test at line 1577 already references the tool.
- **[archive_only_evidence]** sources: .adv/specs/advance-workflow rq (temporal-required mutation freshness): Law: 'Cached status or health evidence is not the sole authority for mutation readiness'; temporal-required target mutation must re-probe with a fresh mutation-boundary probe. Design KD5/DDC2 (fresh target queue evidence only, cached stale not accepted) complies.
- **[archive_only_evidence]** sources: .adv/specs/advance-workflow rq (target external-root derivation): Law: target ADV state root derives from target project id, not caller shard. Design DDC6 + reuse of store.paths.external/getProjectId(context.root) complies; no source identity used for target queue.
- **[archive_only_evidence]** sources: .adv/specs/advance-workflow rq-acWarrant01 (capability warrant): Law: only DECLARED [warrant:] tags are verified at mint; undeclared warrant does not fail mint (proportionality, rq-acWarrant01.1/.3). Agreement AC1/AC2 presume adv_temporal_worker_restart#target_path — the arg this change builds — with no declared warrant tag; contract minted (27 items). Compliant: the AC describes the end-state being built, not a pre-existing surface.
- **[archive_only_evidence]** sources: .adv/specs/advance-meta rq-statusCliWorkerFree01 / rq-dashboardWorkerFree01: Canonical fail-closed precedent: live-Temporal reads fail closed; disk projections must not substitute for live active state. Supports KD6 disk_snapshot_non_authoritative authority metadata for ALL snapshot-ok reads (trusted and untrusted), satisfying AC5/DONT2.
